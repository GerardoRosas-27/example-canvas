...
let marcaContradiccionSignoPositivo = false;
let puntosCorrectosSignoPositivo = false;
...
function validarSignoPositivo() {
  if (pointsSignoPositivo && pointsSignoPositivo.length > 0) {
    ...
    if (aciertos.length === 3 && yPositiva.length === 0) {
      swal({
        title: "Correcto!",
        text: "La función esta genial. Marca con un punto la contradiccion",
        icon: "success",
      });
      marcaContradiccionSignoPositivo = true;
    } else {
      swal({
        title: "Incorrecto!",
...
var getMouseCoordsSignoPositivo2 = function (e, i) {
  var cPos = boardSignoPositivo.getCoordsTopLeftCorner(e, i),
    absPos = JXG.getPosition(e, i),
    dx = absPos[0] - cPos[0],
    dy = absPos[1] - cPos[1];

  return new JXG.Coords(JXG.COORDS_BY_SCREEN, [dx, dy], boardSignoPositivo);
},
  downSignoPositivo2 = function (e) {
    console.log("entro downSignoPositivo2: ", marcaContradiccionSignoPositivo)
    if (marcaContradiccionSignoPositivo) {
      if (!puntosCorrectosSignoPositivo) {
        var canCreate = true,
          i,
          coords,
          el;
        ...
        coords = getMouseCoordsSignoPositivo2(e, i);

        for (el in boardSignoPositivo.objects) {
          if (
            JXG.isPoint(boardSignoPositivo.objects[el]) &&
            boardSignoPositivo.objects[el].hasPoint(
              coords.scrCoords[1],
              coords.scrCoords[2]
            )
          ) {
          ...
          let puntos = boardSignoPositivo.objects[el].hasPoint(
            coords.scrCoords[1],
            coords.scrCoords[2]
          );
        }
        ...
        if (canCreate) {
          if (
            coords.usrCoords[1] > 1 &&
            coords.usrCoords[1] < 2 &&
            coords.usrCoords[2] < 0.02 &&
            coords.usrCoords[2] > -0.02 &&
            puntosCorrectosSignoPositivo == false
          ) {
            puntosCorrectosSignoPositivo = true;
          ...
          let newPunto = boardSignoPositivo.create("point", [
            coords.usrCoords[1],
            coords.usrCoords[2],
          ],{fixed: true});
        }
  ...
  boardSignoPositivo.on("down", downSignoPositivo2);
